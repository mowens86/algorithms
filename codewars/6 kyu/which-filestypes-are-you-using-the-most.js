const solve = (files) => {
    ...
    files.forEach(file => {
      ...
      if (fileHolder[fileExt] > 0) {
        fileHolder[fileExt] = fileHolder[fileExt] + 1;
      }
      
      if (!fileHolder[fileExt]) {
        fileHolder[fileExt] = counter;
      }
    });
    
    const sortedFileNames = Object.entries(fileHolder).sort((a,b) => b[1] - a[1]);
    const highestValue = sortedFileNames[0][1];
    const mostFileNames = sortedFileNames.filter(file => file[1] === highestValue).sort();
    const answer = mostFileNames.map(file => `.${file[0]}`);
    return answer;
  };
   ...
   * Description
    ...
    Given a List/Array of Filenames (strings) files return a List/Array of string(s) contatining the most
     common extension(s). If there is a tie, return a sorted list of all extensions.
