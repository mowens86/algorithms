const tribonacci = (signature,n) => {
    // Conditional for length of N before copying array
    if (n === 0) return [];
    if (n === 1) return [signature[0]];
    if (n === 2) return [signature[0], signature[1]]
    
    // Copy array after conditional checks
    const answer = [...signature];
    
    // While loop to check if answer.length is less than n
     while (answer.length < n) {
       // If true, slice the last 3 of the array, add them up, and put on the end of the answer array
       answer.push(answer.slice(-3).reduce((acc, val) => acc + val));
     }
    return answer;
  };
