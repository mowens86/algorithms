const isValidWalk = (walk) => {
    ...
    walk.forEach(direction => {
      // Conditionals
      if (direction === 'n') northSouthCount++;
      if (direction === 's') northSouthCount--;
      if (direction === 'e') eastWestCount++;
      if (direction === 'w') eastWestCount--;
      // Add one minute
      totalMinutes++;
    });
    // Return all values and if all pass return true, else false
    return totalMinutes === 10 && northSouthCount === 0 && eastWestCount === 0 ? true : false;
  };
   ...
   *  one city block, so create a function that will return true if the walk the app gives you will take you exactly ten
